...
export class YouTubeHomePage extends BasePage {
  ...
  private async dismissConsentIfPresent(): Promise<void> {
    const reject = this.page.getByRole('button', { name: /^(reject all|відхилити всі)$/i });
    const accept = this.page.getByRole('button', { name: /^(accept all|прийняти всі)$/i });

    try {
      await reject.first().click({ timeout: 4000 });
      return;
    } catch {
    }
